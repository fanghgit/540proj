...
def remap_label_hierarchy(input_file, output, label_dict, feature_dict, label_start_flag, feature_start_flag, remap_flag):
    f = open(input_file, 'r')
    g = open(output, 'w')
    cur_label = len(label_dict)
    for line in f:
        ...
        if len(tmp) > 2:
            ...
            for lbl in tmp:
                if lbl in label_dict:
                    ...
                    cur_label += 1
                    label_dict[lbl] = str(cur_label)
                    lbl = str(cur_label)
                new_tmp.append(lbl)
            newline = " ".join(new_tmp)
            #sys.exit("more than 2 labels in label hierarchy file at line " + str(line_number))
        else:
            lbl1 = tmp[0]
            lbl2 = tmp[1]
            if lbl1 in label_dict:
                lbl1 = label_dict[lbl1]
            else:
                cur_label += 1
                label_dict[lbl1] = str(cur_label)
                lbl1 = str(cur_label)
            if lbl2 in label_dict:
                lbl2 = label_dict[lbl2]
            else:
                cur_label += 1
                label_dict[lbl2] = str(cur_label)
                lbl2 = str(cur_label)
            newline = lbl1 + " " + lbl2
        print(newline, file = g)
    f.close()
    g.close()
    print("total number of labels hierarchy: ", cur_label)
...
if __name__ == "__main__":
    ...
    label_dict = remap_test(test_data, output_test, label_dict, feature_dict, label_start_flag, feature_start_flag, remap_flag)
    print("testing remap finished!")
    remap_label_hierarchy(label_hierarchy_input, label_hierarchy_output, label_dict, feature_dict, label_start_flag, feature_start_flag, remap_flag)
    print("label hierarchy remap finished!")
